  ...
  return (
    <Reveal delay={delay}>
      <div className="rounded-2xl border border-ink/10 bg-white p-6 transition hover:-translate-y-1 hover:shadow-lg">
        ...
        <h3 className="mt-2 text-xl font-semibold">{e.position}</h3>
        <p className="mt-1 text-ink/75">{e.company_name}</p>
        <p className="mt-3 text-sm text-ink/60">{range}</p>
        {e.description && (
          <p className="mt-4 leading-relaxed text-ink/80">{e.description}</p>
        )}
      </div>
    </Reveal>
  )
